...
export type Props = {
  // 表单中需要编辑的数据
  values: API.UpdateInterfaceParams;
  columns: ProColumns<API.CreateInterfaceParams>[];
  // 当用户点击取消按钮时触发
  onCancel: () => void;
  // onCancel: (flag?: boolean, formVals) => void;
  // 当用户提交表单时，将用户输入的数据作为参数传递给后台
  onSubmit: (values: API.CreateInterfaceParams) => Promise<void>;
  // 模态框是否可见
  visible: boolean;
...
const UpdateModal: React.FC<Props> = (props) => {
  ...
  return (
    // 创建一个Modal组件，通过visible属性控制其显式或隐藏，footer设置为null 把表单项的‘取消’和‘确认’按钮去掉
